...
function promptChangeDevice(): boolean {
...
export default function Login() {
  ...
  const attemptLogin = async (forceDeviceTakeover: boolean) => {

    await login(email, password, { rememberMe, forceDeviceTakeover });
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isDeviceSessionConflictError(err)) {
        const change = promptChangeDevice();
        if (change) {
          try {
            ...
            const retryMessage =
              retryErr instanceof Error
                ? retryErr.message
                : "Could not switch to this device. Try again.";
              ...
              {error ? (
                ...
                <p
                  ...
                  className={`rounded-md border px-3 py-2 text-sm ${
                    ...
                    /cancelled/i.test(error)

                      ? "border-[#FFB347]/40 bg-[#FFF8EE] text-[#8A6D3B]"
